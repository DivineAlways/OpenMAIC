  ...
  const handleGoLive = async () => {
    ...
    try {
      ...
      const createRes = await fetch("/api/live/create-room", {
        ...
        body: JSON.stringify({ session_id: selectedSession, stream_type: streamType, user_id: userId }),
      })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-md p-6 space-y-5">
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold uppercase tracking-widest text-zinc-400">Session</label>
          {sessions.length === 0 ? (
            <p className="text-sm text-zinc-500">No sessions scheduled. Create one from the main platform admin panel first.</p>
          ) : (
            <select
            ...
            >
              <option value="" className="bg-zinc-900">— pick a session —</option>
              {sessions.map((s) => (
        ...
        {/* Stream type */}
        <div className="space-y-2">
          <label className="text-xs font-bold uppercase tracking-widest text-zinc-400">Stream Type</label>
          <div className="grid grid-cols-3 gap-2">
            {(Object.entries(STREAM_CONFIGS) as [StreamType, typeof STREAM_CONFIGS[StreamType]][]).map(([type, cfg]) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{cfg.label}</span>
                  <span className="text-[10px] font-normal text-zinc-500">max {cfg.max_participants}</span>
                </button>
              )
            })}
          </div>
