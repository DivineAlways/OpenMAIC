...
export async function POST(req: NextRequest) {
  ...
  const { session_id, stream_type = "broadcast", user_id } = body as {
    session_id?: string
  ...
  if (!["1on1", "group", "broadcast"].includes(stream_type)) {
    return NextResponse.json({ error: "stream_type must be 1on1, group, or broadcast" }, { status: 400 })
  }
  ...
  if (!user.is_paid) {
    return NextResponse.json({ error: "Active membership required to host live sessions" }, { status: 403 })
  }
